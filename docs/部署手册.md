# 部署手册

## 1. 部署方式

推荐方式只有一种：

- 直接拉取 GitHub Actions 已构建好的 GHCR 镜像部署

不再推荐业务环境中本地构建镜像或手动跑 Node.js 进程。

如果你要调整业务映射规则，另外参考：

- [report-rules 配置说明](/Users/xiaochen/Desktop/123/docs/report-rules配置说明.md)

## 2. 架构图

```mermaid
flowchart LR
    subgraph Client["客户端侧"]
        Browser["业务用户浏览器<br/>页面入口 /"]
        Codex["Codex / AI 客户端"]
        Skill["项目 Skill"]
        MCP["本地 stdio MCP"]
    end

    subgraph Server["服务端侧"]
        API["报表 HTTP 服务<br/>Docker 容器"]
        Files["生成报表目录<br/>OUTPUT_DIR"]
        Oracle["Oracle 数据库"]
        Template["Excel 模板<br/>TEMPLATE_PATH"]
        Rules["规则配置<br/>REPORT_RULES_PATH"]
    end

    Browser -->|HTTP| API
    Codex --> Skill
    Skill -->|HTTP API| API
    Codex --> MCP
    MCP -->|HTTP API| API
    API --> Oracle
    API --> Template
    API --> Rules
    API --> Files
```

说明：

- 集中部署的是报表 HTTP 服务
- 业务用户优先走页面入口
- Skill 和 MCP 都只是调用 HTTP 服务，不直接连数据库
- 当前 MCP 是本地 `stdio` MCP，不是远程 MCP 服务

## 3. 拉取镜像

推荐直接拉东八区时间标签：

```bash
docker pull ghcr.io/xiaoguan521/baobiao:baobiao-202604170910
```

也可以拉最新版本：

```bash
docker pull ghcr.io/xiaoguan521/baobiao:latest
```

## 4. 需要调整的配置项

部署时重点就是改下面这些环境变量。

| 变量名 | 必填 | 说明 | 示例 |
|---|---|---|---|
| `PORT` | 否 | 服务监听端口，默认 `3000` | `3000` |
| `ORACLE_USER` | 是 | Oracle 用户名 | `damoxing` |
| `ORACLE_PASSWORD` | 是 | Oracle 密码 | `your-password` |
| `ORACLE_DSN` | 是 | Oracle 连接串 | `host.docker.internal:51521/FREEPDB1` |
| `TEMPLATE_PATH` | 否 | 模板路径，默认 `/app/模板.xlsx` | `/app/模板.xlsx` |
| `OUTPUT_DIR` | 否 | 输出目录，默认 `/app/generated` | `/app/generated` |
| `DOWNLOAD_ROOT` | 否 | 下载目录，默认跟 `OUTPUT_DIR` 一致 | `/app/generated` |
| `REPORT_RULES_PATH` | 否 | 规则文件路径，默认 `/app/config/report-rules.json` | `/app/config/report-rules.json` |
| `REPORT_API_TOKEN` | 建议 | 生成/下载/调试接口鉴权 token | `replace-with-a-strong-token` |
| `REPORT_PUBLIC_BASE_URL` | 建议 | 对外访问服务的完整地址 | `https://report.example.com` |

重点说明：

- `ORACLE_DSN` 要写成“从容器内部可访问”的地址，不一定和你本机测试时一样
- 如果 Oracle 也是容器并且已经映射到宿主机端口，常见写法可以直接用 `host.docker.internal:宿主机端口/服务名`
- 你当前这类 `gvenzl/oracle-free` 容器，常见服务名通常是 `FREEPDB1`，不要继续写成 `127.0.0.1:11521/orcl`
- `OUTPUT_DIR` 和 `DOWNLOAD_ROOT` 建议保持一致
- `REPORT_PUBLIC_BASE_URL` 建议写成最终用户访问的域名或网关地址，它主要用于生成接口返回的对外下载链接
- 如果使用仓库里的 `deploy/docker-compose.yml` 或 `deploy/docker-compose.example.yml`，`config/report-rules.json` 会作为宿主机文件挂载到容器里，运维人员直接修改宿主机上的这份文件即可
- 由于 `docker-compose` 文件位于 `deploy/` 目录下，挂载写法 `../config/report-rules.json:/app/config/report-rules.json:ro` 实际对应仓库根目录下的 `config/report-rules.json`
- 如果直接使用镜像内置目录结构，`TEMPLATE_PATH`、`OUTPUT_DIR`、`DOWNLOAD_ROOT` 通常都不用额外配置
- `REPORT_RULES_PATH` 对应的文件说明见：
  - [report-rules 配置说明](/Users/xiaochen/Desktop/123/docs/report-rules配置说明.md)
- 一旦设置 `REPORT_API_TOKEN`，以下接口就需要带 token：
  - `POST /api/reports/generate`
  - `GET /api/reports/debug/unmatched`
  - `GET /api/reports/download/:fileId`

## 5. 推荐部署命令

下面这份命令是最简部署模板，优先推荐：

```bash
docker run -d \
  --name baobiao \
  -p 3000:3000 \
  -e ORACLE_USER=damoxing \
  -e ORACLE_PASSWORD='your-password' \
  -e ORACLE_DSN='host.docker.internal:51521/FREEPDB1' \
  -e REPORT_RULES_PATH='/app/config/report-rules.json' \
  -e REPORT_API_TOKEN='replace-with-a-strong-token' \
  -e REPORT_PUBLIC_BASE_URL='https://report.example.com' \
  -v /data/baobiao/generated:/app/generated \
  -v /data/baobiao/report-rules.json:/app/config/report-rules.json:ro \
  ghcr.io/xiaoguan521/baobiao:baobiao-202604170910
```

你通常只需要改这几类值：

- Oracle 连接信息
- 对外访问域名
- token
- 宿主机挂载目录
- 镜像 tag

### 5.1 docker-compose 示例

如果你习惯用 `docker compose` 管理服务，可以直接参考仓库里的示例文件：

```text
deploy/docker-compose.example.yml
```

内容如下：

```yaml
services:
  baobiao:
    image: ghcr.io/xiaoguan521/baobiao:baobiao-202604170910
    container_name: baobiao
    restart: unless-stopped
    ports:
      - "3000:3000"
    environment:
      ORACLE_USER: damoxing
      ORACLE_PASSWORD: your-password
      ORACLE_DSN: host.docker.internal:51521/FREEPDB1
      REPORT_RULES_PATH: /app/config/report-rules.json
      REPORT_API_TOKEN: replace-with-a-strong-token
      REPORT_PUBLIC_BASE_URL: https://report.example.com
    volumes:
      - ./data:/app/generated
      - ../config/report-rules.json:/app/config/report-rules.json:ro
```

启动命令：

```bash
docker compose -f deploy/docker-compose.example.yml up -d
```

其中规则文件挂载这一行：

```yaml
      - ../config/report-rules.json:/app/config/report-rules.json:ro
```

表示：

- 左侧是宿主机文件
- 右侧是容器内路径
- `:ro` 表示只读挂载，避免容器内误修改

如果你需要覆盖镜像默认路径，也可以在 `environment` 里额外补：

```yaml
      TEMPLATE_PATH: /app/模板.xlsx
      OUTPUT_DIR: /app/generated
      DOWNLOAD_ROOT: /app/generated
      REPORT_RULES_PATH: /app/config/report-rules.json
```

如果你确实要覆盖镜像默认路径，再额外补这些配置：

```bash
-e TEMPLATE_PATH='/app/模板.xlsx' \
-e OUTPUT_DIR='/app/generated' \
-e DOWNLOAD_ROOT='/app/generated' \
-e REPORT_RULES_PATH='/app/config/report-rules.json' \
```

## 6. Nginx 反向代理

如果这个容器是独立部署的，推荐在前面加一层 Nginx 反向代理。

最推荐的方式是使用独立子域名，例如：

```text
https://report.example.com  ->  http://127.0.0.1:3000
```

这样最简单，因为当前页面和 API 都按根路径设计：

- `/`
- `/styles.css`
- `/app.js`
- `/api/...`

### 6.1 容器环境变量

如果前面有 Nginx，请把：

```text
REPORT_PUBLIC_BASE_URL
```

设置成最终用户访问的地址，例如：

```text
https://report.example.com
```

这样生成接口返回的 `file.downloadUrl` 才会是正确的外网地址。

### 6.2 Nginx 配置示例

仓库内已提供示例文件：

```text
deploy/nginx.example.conf
```

HTTP 示例：

```nginx
server {
    listen 80;
    server_name report.example.com;

    client_max_body_size 20m;

    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;

        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;

        proxy_read_timeout 300s;
        proxy_send_timeout 300s;
    }
}
```

HTTPS 示例：

```nginx
server {
    listen 443 ssl http2;
    server_name report.example.com;

    ssl_certificate /path/to/fullchain.pem;
    ssl_certificate_key /path/to/privkey.pem;

    client_max_body_size 20m;

    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;

        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;

        proxy_read_timeout 300s;
        proxy_send_timeout 300s;
    }
}
```

说明：

- 当前程序已经支持代理头识别
- 反向代理时务必保留 `Host` 和 `X-Forwarded-Proto`
- 不建议一开始就挂在 `/baobiao/` 这类二级路径下，优先使用独立子域名

## 7. 部署后怎么验证

### 7.1 健康检查

```bash
curl http://127.0.0.1:3000/api/health
```

如果已经接了 Nginx，也可以直接验证对外地址：

```bash
curl https://report.example.com/api/health
```

### 7.2 页面入口

浏览器访问：

```text
http://127.0.0.1:3000/
```

如果已经接了 Nginx，则访问：

```text
https://report.example.com/
```

### 7.3 生成报表

```bash
curl -X POST http://127.0.0.1:3000/api/reports/generate \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer replace-with-a-strong-token' \
  -d '{"month":"2025-12"}'
```

### 7.4 下载文件

```bash
FILE_ID='把生成接口返回的 file.id 填到这里'
curl -L "http://127.0.0.1:3000/api/reports/download/${FILE_ID}" \
  -H 'Authorization: Bearer replace-with-a-strong-token' \
  -o report.xlsx
```

## 8. MCP 说明

当前仓库里的 MCP 是本地 `stdio` MCP：

```bash
REPORT_API_BASE_URL='https://report.example.com' \
REPORT_API_TOKEN='replace-with-a-strong-token' \
npm run mcp
```

它不是服务器上统一部署的远程 MCP 网关，只适合运行在 AI 客户端所在机器上。

如果只是给业务用户使用，不需要 MCP，直接用页面入口即可。
